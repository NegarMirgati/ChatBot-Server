...
class block_helloworld extends block_base
{
    ...
    public function get_content()
    {
        ...
        if (count($helloworldpages)) {
           $this->content->text = '<iframe src="http://localhost:5000/" width=100% heigth=600px></iframe>';
...
//             }
          ...
          //$this->content->text = '<form action="chatpage.htm"><input type="submit" value="chat"></form>';
            ...
            <iframe  src="http://localhost:5000/'.$USER->id.'"></iframe>';
            ...
            // $myfile = fopen("/blocks/helloworld/info.txt", "w");
            // $this->content->text = fopen("/blocks/helloworld/info.txt", "x");
            // fwrite($myfile, $current);
            // //fwrite($myfile, $course_);
            // fclose($myfile);
        }
        if (has_capability('block/helloworld:managepages', $context)) {
